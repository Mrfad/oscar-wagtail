...
class CatalogueView(BaseSearchView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        ctx["object_list"] = Product.objects.filter(is_public=True) # just added this line
        return ctx
...
#         from oscar.apps.catalogue.models import Product
#         ctx["products"] = Product.objects.filter(is_public=True)
#         ctx["object_list"] = ctx["products"]  # Explicitly set object_list

#         print(f"✅ Updated context keys: {ctx.keys()}")
#         print(f"✅ Final products in template: {list(ctx['object_list'].values_list('title', flat=True))}")
